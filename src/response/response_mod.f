...
contains
   ...
   do i_chain = 1, n_chain
      ...
      do i_order = 0, order
         ...
         call make_chain_grid(chains0(i_order+1,i_chain),plan,nblk,&
              block_length(1:nblk,i_chain),block_type(1:nblk,i_chain),ds(:,i_chain),&
   ...
   do i_chain = 1,N_chain
      do i_order = 0, order
         blk_end = chains0(i_order+1,i_chain)%block_bgn_lst(2,N_block(i_chain))
      ...
      blk_end = pchains(i_chain)%block_bgn_lst(2,N_block(i_chain))
   ...
   do i_chain = 1, N_chain
      ...
      do i_blk = 1, N_block(i_chain)
         ...
         blk_bgn = pchains(i_chain)%block_bgn_lst(1,i_blk)
         blk_end = pchains(i_chain)%block_bgn_lst(2,i_blk)
     ...
     !---------------------------------------------
      do i_blk = N_block(i_chain), 1, -1
         ...
         blk_bgn = pchains(i_chain)%block_bgn_lst(2,i_blk)
         blk_end = pchains(i_chain)%block_bgn_lst(1,i_blk)
 ...
 200     continue
   ...
   do sp_index = 1,n_chain
      do iblk = 1,n_block(sp_index)
         ...
         i_bgn = pchains(sp_index)%block_bgn_lst(1,iblk)
         i_end = pchains(sp_index)%block_bgn_lst(2,iblk)
         i = i_bgn
