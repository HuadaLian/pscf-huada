...
!*** ------------------------------------------------------------------
!****v* chemistry_mod/block_type
! VARIABLE
!    integer block_type(:,:) - dimension(N_blk_max,N_chain) 
!            block_type(i,j) - chain type of block i, species j
!*** ------------------------------------------------------------------
...
contains
   ...
   case('F')
      ...
      if (N_chain > 0) then
         ...
         call output(trim(comment_line),f='N',j='L')
         do j = 1, N_chain
            call input(block_type(:,j),N_block(j),c='block_type',f='N')
         enddo
   ...
   case('F')
      ...
      if (N_chain > 0) then
         ...
         call output(trim(comment_line),f='N',j='L')
         do j = 1, N_chain
            call input(block_type(:,j),N_block(j),f='N')
         enddo
         read(i_unit,*) comment_line
         call output(trim(comment_line),f='N',j='L')
         do j = 1, N_chain
   ...
   case('U')
      ...
      if (N_chain > 0) then
         ...
         enddo
         do j=1, N_chain
            read(i_unit) block_type(1:N_block(j),j)
         enddo
         do j=1, N_chain
            read(i_unit) block_length(1:N_block(j),j)
         enddo
   ...
   case('F') ! formatted for input
      ...
      if (N_chain > 0) then
         ...
         enddo
         call output('block_type',f='N',j='L')
         do j = 1,N_chain
            call output(block_type(:,j),N_block(j),f='N')
         enddo
   ...
   case('U')
      ...
      if (N_chain > 0) then
         ...
         enddo
         do j=1, N_chain
            write(o_unit) block_type(1:N_block(j),j)
         enddo
