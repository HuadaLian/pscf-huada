...
module group_mod
   ...
   !       complex(long) large_complex_matrix_inverse(:,:)
   !       real(long)    large_matrix_inverse(:,:)
   ...
   interface inverse
      ...
      module procedure large_matrix_inverse ! using LAPACK function 
      module procedure large_complex_matrix_inverse ! using LAPACK function
   end interface
   !***
...
contains
   ...
   !-------------------------------------------------------------------
   !****ip group_mod/large_complex_matrix_inverse
   ! FUNCTION
   !   large_matrix_inverse(m,N)
   !   complex(long) dimension(N,N) :: matrix_inverse(N,N)
   ! RETURN
   !   Return an inverse of a complex matrix calculated by finding the LU
   ! SOURCE
   !-------------------------------------------------------------------
   function large_complex_matrix_inverse(m,d) 
      complex      , intent(IN)        :: m(:,:)
      integer,intent(IN)               :: d 
      complex                          :: large_complex_matrix_inverse(size(m,1),size(m,2))
      complex                          :: work(size(m,1))        ! work array for LAPACK
      integer                          :: ipiv(size(m,1))          ! pivot indices 
      integer                          :: info 
      integer                          :: j 
      ...
      if (d /= size(m,1) .OR. d /= size(m,2)) then
         stop "Matrix dimension doesn't match with input parameter N."  
      endif 

      ! Store m in minv to prevent it from being overwritten by LAPACK 
      large_complex_matrix_inverse = m 

      ! CGETRF computes an LU factorization of a general M-by-N matrix m 
      ! using partial pivoting with row interchanges 
      call CGETRF(d,d,large_complex_matrix_inverse ,d,ipiv,info)
      if (info /= 0) then 
         do j=1,size(m,1) 
            print *, j,m(j,j) 
         enddo
      ...
      call CGETRI(d,large_complex_matrix_inverse,d,ipiv,work,d,info) 
      if (info /= 0) then 
         print *, large_complex_matrix_inverse
         stop 'Matrix inversion failed!'
      endif

   end function large_complex_matrix_inverse 

   !-------------------------------------------------------------------
   !****ip group_mod/large_matrix_inverse
   ! FUNCTION
   !   large_matrix_inverse(m,N)
   !   real(long) dimension(N,N) :: matrix_inverse(N,N)
   ! RETURN
   !   Return an inverse of a matrix calculated by finding the LU
   ! SOURCE
   !-------------------------------------------------------------------
   function large_matrix_inverse(m,d) 
      real(long), dimension(:,:), intent(IN) :: m 
      integer,intent(IN)                        :: d 
      real(long), dimension(size(m,1),size(m,2))  :: large_matrix_inverse

      real(long),dimension(d) :: work        ! work array for LAPACK
      integer, dimension(d) :: ipiv          ! pivot indices 
      integer :: info 
      ...
      ! Store m in minv to prevent it from being overwritten by LAPACK 
      large_matrix_inverse = m

      ! DGETRF computes an LU factorization of a general M-by-N matrix m 
      ! using partial pivoting with row interchanges 
      call DGETRF(d,d,large_matrix_inverse,d,ipiv,info)
      if (info /= 0) then 
         print *, m(1,1), m(2,2), m(3,3), m(4,4) 
         stop 'Matrix is numerically singular!'
      endif

      ! DGETRI computes the inverse of a matrix using the LU factorization 
      call DGETRI(d,large_matrix_inverse,d,ipiv,work,d,info) 
      if (info /= 0) then 
         stop 'Matrix inversion failed!'
      endif

   end function large_matrix_inverse 
